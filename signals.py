...
def random_signal(n: int, display_plot: bool = False):  
    ...
    if display_plot:
        ...
        ax.set_title(f'Szereg Czasowy z sygnału losowego (pierwsze 500 próbek)')
        ax.plot(data[:500], color='black', label=f'Sygnału losowy', linewidth=0.8)
        plt.show()
...
def periodic_signal(n: int, display_plot: bool = False):
    ...
    if display_plot:
        fig = plt.figure(figsize=(8, 6))
        ax = fig.add_subplot(111)
        ax.set_title(f'Szereg Czasowy z sygnału periodycznego (pierwsze 500 próbek)')
        ax.plot(data[:500], color='black', label=f'Sygnał periodyczny', linewidth=0.8)
        plt.show()
